...
**ICL（成都高新减灾技术研究所 / 大陆地震预警网）数据源的所有 API 信息、URL、接入方式，不论任何形式、任何方式、任何原因，一律不得上传至 GitHub 或其他公开仓库！**
**此数据源在法律上属于未公开/非授权数据，公开传播可能触犯刑法！**

**违规后果：** 如果该数据源的 API 细节因本仓库被公开传播，导致法律追责，一切后果由违规者自行承担。

**Git commit 前必须检查：** 确保 `main.py`、`parser/`、`config/` 中没有任何 ICL 的 API URL、密钥、接入方式的明文记录。ICL 相关的配置只存在于本地运行目录（AppData），不得出现在 Git 跟踪的任何文件中。
