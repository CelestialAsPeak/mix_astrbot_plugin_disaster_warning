...
class MapAttachmentBuilder:
    ...
    async def render_map_image(
        ...
        try:
            ...
            logger.info(f"[MIX_MAP] map_source={map_source!r} tile_url={get_tile_url_js(map_source)!r} lat={lat} lon={lon} zoom={zoom_level}")
            ...
            if playwright_mode == "remote":
                ...
                leaflet_js_url = Path(leaflet_path).as_uri()
                leaflet_css_url = Path(leaflet_css_path).as_uri()
